...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, pos_embed=None, mask=None) -> torch.Tensor:
        residual = src
        if self.normalize_before:
            src = self.norm1(src)
        q = k = self.with_pos_embed(src, pos_embed)
        src, _ = self.self_attn(q, k, value=src, key_padding_mask=mask)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src, pos_embed=None, mask=None):
        """
        Args:
            src (torch.Tensor): [2 * N, L, C]
            pos_embed (torch.Tensor): [N, L, C] (optional)
            mask (torch.Tensor): [N, L] (optional)
        """
        mask_ = torch.cat([mask, mask], dim=0)
        for layer in self.layers:
            src = layer(src, pos_embed, mask_)
        return src
...
class HybridEncoder(nn.Module):
    ...
    @staticmethod
    def position_embedding(w, h, embed_dim=256, temperature=10000.):
        ...
        return torch.concat([out_w.sin(), out_w.cos(),
                             out_h.sin(), out_h.cos()], dim=1)[None, :, :]
    ...
    def forward(self, feats, masks):
        ...
        if self.num_encoder_layers > 0:
            for i, enc_ind in enumerate(self.use_encoder_idx):
                ...
                src_flatten = rearrange(
                    proj_feats[enc_ind], 'b c h w -> b (h w) c')
                mask_flatten = rearrange(
                    masks[enc_ind], 'b h w -> b (h w)')
                ...
                if self.training or self.eval_spatial_size is None:
                    ...
                    pos_embed = getattr(
                        self, f'pos_embed{enc_ind}', None
                    ).to(src_flatten.device)

                memory = self.encoder[i](
                    src_flatten, pos_embed=pos_embed, mask=mask_flatten)
                proj_feats[enc_ind] = rearrange(
                    memory, 'b (h w) c -> b c h w', h=h, w=w).contiguous()
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            feat_high = \
                self.lateral_convs[len(self.in_channels) - 1 - idx](feat_high)
